...
const PostForm = ({ initialPost, busy, postBtnTitle, handleCreatePost, resetAfterSubmit }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const postTags = post.tags.map((tag) => tag.trim()).splice(0, 4); // Only four tags are allowed
        ...
        <form className='p-2 flex' onSubmit={handleSubmit}>
            <div className="w-9/12 h-screen space-y-3 flex flex-col">
                ...
                <div className="flex items-center space-x-2 mt-5">
                    <input type="checkbox" value={post.featured} name="featured" onChange={handleChange} id="featured" />
                ...
                {/* Title */}
                <input type="text" value={post.title} onChange={handleChange} onFocus={() => setDisplayMarkdownHints(false)} placeholder='Post Title' name="title" id="title" className='text-xl font-semibold w-full h-10 px-3 border-2 border-gray-300 rounded focus:outline-none focus:border-blue-500' />
                {/* Image Input */}
                ...
                {/* Content */}
                <textarea value={post.content} onChange={handleChange} onFocus={() => setDisplayMarkdownHints(true)} placeholder='## Markdown' name="content" id="content" cols="30" rows="10" className='flex-1 w-full p-2 h-96 border-2 font-mono tracking-wide text-lg border-gray-300 rounded focus:outline-none focus:border-blue-500 resize-none'></textarea>
                {/* Tags Input */}
                <div>
